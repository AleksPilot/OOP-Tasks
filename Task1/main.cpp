...
int main(int argc, char** argv) {
    ...
    while (getline(in_file, str_from_file)) {
        for (int i = 0; i < str_from_file.length(); i++) { //if the symbol is not the spliter, adding him to the end of str
            ...
            else if (word.length() == 0) { //if the words' length is equal to 0, doing nothing
                continue;
            }
            else { //if the symbol is spliter then appending the written word to the list and making word zero
                list_of_words.push_back(word);
                word.clear();
                num_of_words += 1;
            }
        }
        if (word.length() > 0) { //at the end of string, if there is an extra word or symbol adding him to the end of list
            list_of_words.push_back(word);
            word.clear();
            num_of_words += 1;
        }
